...
export default function Table() {
  ...
  const tableData = CustomeHookTable("orders_table", setLoading);

  const notPaid = tableData?.filter(
    (item) => item.payment_status === "not paid",
  );
  useEffect(() => {
    const totalUnpaid = notPaid.reduce((acc, item) => acc + item.t_amount, 0);
    setTotalUnpaidAmount(totalUnpaid);
  }, [notPaid]);

  const paid = tableData?.filter((item) => item.payment_status === "paid");

  useEffect(() => {
    const totalPaid = paid.reduce((acc, item) => acc + item.t_amount, 0);
  ...
  return (
    <>
      {loading ? (
        ...
        <>
          <Pagination heading="Order" page={"Table"} />
                ...
                <div className="col-xl-12">
                  <div className="card">
                    <div className="card-header">
                      <h6 className="card-title mb-0">Order Account Manager</h6>
